const {
  cleanupRooms,
  getRoom,
  touch,
  getPlayerNumber,
  checkWin,
  isDraw,
} = require("./store");

module.exports = (req, res) => {
  if (req.method !== "POST") {
    res.statusCode = 405;
    res.end("Method Not Allowed");
    return;
  }
  cleanupRooms();
  const body = typeof req.body === "string" ? JSON.parse(req.body || "{}") : req.body || {};
  const code = (body.code || "").toString().trim().toUpperCase();
  const playerId = (body.playerId || "").toString();
  const x = Number(body.x);
  const y = Number(body.y);
  if (!code || !playerId || Number.isNaN(x) || Number.isNaN(y)) {
    res.statusCode = 400;
    res.end("参数缺失");
    return;
  }
  const room = getRoom(code);
  if (!room) {
    res.statusCode = 404;
    res.end("房间不存在");
    return;
  }
  const playerNumber = getPlayerNumber(room, playerId);
  if (!playerNumber) {
    res.statusCode = 403;
    res.end("无权限");
    return;
  }
  if (!room.started) {
    res.statusCode = 409;
    res.end("等待对手加入");
    return;
  }
  if (room.winner) {
    res.statusCode = 409;
    res.end("对局已结束");
    return;
  }
  if (room.currentPlayer !== playerNumber) {
    res.statusCode = 409;
    res.end("不是你的回合");
    return;
  }
  if (x < 0 || y < 0 || x >= room.board.length || y >= room.board.length) {
    res.statusCode = 400;
    res.end("坐标无效");
    return;
  }
  if (room.board[x][y] !== 0) {
    res.statusCode = 409;
    res.end("该位置已有棋子");
    return;
  }
  room.board[x][y] = playerNumber;
  room.lastMove = { x, y, player: playerNumber };
  if (checkWin(room.board, x, y, playerNumber)) {
    room.winner = playerNumber;
  } else if (isDraw(room.board)) {
    room.winner = -1;
  } else {
    room.currentPlayer = playerNumber === 1 ? 2 : 1;
  }
  touch(room);
  res.setHeader("Content-Type", "application/json");
  res.end(
    JSON.stringify({
      code: room.code,
      board: room.board,
      currentPlayer: room.currentPlayer,
      started: room.started,
      winner: room.winner,
      lastMove: room.lastMove,
    })
  );
};
